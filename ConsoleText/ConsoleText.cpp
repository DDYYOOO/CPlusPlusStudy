...
int main()
{
    ...
    StringCchPrintf(buf, 256, TEXT("%s 콘솔 텍스트 게임 버전 %.2f"), _T("이동윤"), 1.00f);
    ...
    {
        system("cls");
        RenderGame(player, world);      // 1. Rendering : 이미지(맵)를 그려주는 것
        GetInput(player, world);        // 2. 플레이어의 입력
        UpdateGame(player, world);      // 3. Update : 특정 위치에 도착하면 다음 맵으로 이동. 플레이어의 위치, 특정 장소의 위치 같은지 아닌지
